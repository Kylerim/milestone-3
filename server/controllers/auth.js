    ...
    User.findOne({ name }).exec((err, user) => {
        if (user) {
            ...
            const output = `
			 ...
			 `;
            console.log("http://kylerim.cse356.compas.cs.stonybrook.edu/users/verify?key=" + randomKey)
            ...
            transporter.sendMail(mailOptions, (error, info) => {
                ...
                newUser.save((err, success) => {
                    if (err) {
                        res.json({ error: true, message: "error with sign up " + err })
                        return
                    } else {
...
exports.login = (req, res) => {
    ...
    if (req.session.user) {
        ...
        User.findOne({ name: req.session.user }).exec((err, user) => {
            if (err) {
                res.json({ error: true, message: "Error in fetching data from db" })
                return
            }
            else if (!user) {
                req.session.destroy();
                res.json({ error: true, message: "No user with corresponding username" })
                return
            }
            else {
        ...
        if (email && password) {
            ...
            User.findOne({ email }).exec((err, user) => {
                ...
                if (!user) {
                    res.json({ error: true, message: "No user with corresponding username and password" })
                    return
                }
                ...
                if (password != dbPassword) {
                    ...
                    res.json({ error: true, message: "User is not verified yet" })
                    return
                } else {
            ...
            res.json({ error: true, message: "no username/pw provided | no session found yet" })
            return;
...
exports.verify = (req, res) => {
    ...
    User.findOne({ key }).exec((err, user) => {
        console.log(user);
        if (user) {
            User.findOneAndUpdate({ key }, { isVerified: true }, (err, user) => {
                if (!user) {
