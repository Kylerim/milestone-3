...
function TextEditor() {
    ...
    const wrapperRef = useCallback(wrapper => {
        ...
        const q = new Quill(editor, {
            theme: "snow",
            modules: { toolbar: ['bold', 'italic', 'underline', 'strike', 'align'], cursors: true },
        })
    ...
    function handlePresence(data) {
        ...
        setPresences(function (prevState) {
            const index = prevState.findIndex(presence => presenceIn.id === presence.id);
            if (index < 0) {
        ...
        cursors.createCursor(presenceIn.id, presenceIn.cursor.name, presenceIn.color);
    ...
    useEffect(() => {
        ...
        if (!listening) {
            ...
            const events = new EventSource(`${backendURL}/doc/connect/${docId}/${connectionId}`, { withCredentials: true });
            events.onmessage = (event) => {
                const data = JSON.parse(event.data)
                if (data.content) {
                    console.log(`Connected to docId: ${docId} version: ${data.version}`)
                    ...
                    console.log("Content Update from Other Collaborators Received.")
    ...
    }, [listening, quill, presences, cursors, docId])
    ...
    useEffect(() => {
        ...
        const textChangeHandler = async (delta, oldDelta, source) => {
            ...
            const rawResponse = await fetch(`${backendURL}/doc/op/${docId}/${connectionId}`, {
                method: 'POST',
                headers: {
        ...
        const selectionChangeHandler = async (range, oldRange, source) => {
            ...
            // type.
            // console.log(range);
            const rawResponse = await fetch(`${backendURL}/doc/presence/${docId}/${connectionId}`, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
    ...
    return (
        <div>
            ...
            <ul> {presences.map(presence => <li key={presence.id}> {presence.id}</li>)}</ul>
            ...
            {isUploading === 1 ?
                <>
                    <p>Image Upload Complete. Drag the image into the desired position above.</p>
                    <img alt={"Img"} src={uploadedImageURL} width="100%"></img>
                </>
                :
                isUploading === 2 ? <p>Error uploading</p> :
                    isUploading === 0 ? <p>uploading...</p> :
                        isUploading === -1 ? <p>Upload the image</p> :
                            isUploading === -2 ? <p>Choose an Image...</p> : <p>Err.</p>}
        </div>
    );
}
